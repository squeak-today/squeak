...
export type GetContentResponse = components["schemas"]["workspaces.GetContentResponse"];
...
export function useContentAPI() {
  ...
  const getContent = useCallback(async (workspaceId: string, databaseId: string, contentId: string) => {
    return requireAuthWithErrors(async () => {
      ...
      return { data: data as GetContentResponse, error: error as components["schemas"]["models.ErrorResponse"] | null };
    })
