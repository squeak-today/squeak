...
func (p *ContentProcessor) Process(ctx context.Context, job *types.ContentJobRecord) error {
	if err := workspaces.UpsertContentJob(ctx, p.supabaseClient, job.Job.UserID, job.Job.DatabaseID, string(job.Status)); err != nil {
		return err
	}
	...
	case <-time.After(20 * time.Second):
		...
		return workspaces.UpsertContentJob(ctx, p.supabaseClient, job.Job.UserID, job.Job.DatabaseID, string(types.ContentJobStatusComplete))
	case <-ctx.Done():
